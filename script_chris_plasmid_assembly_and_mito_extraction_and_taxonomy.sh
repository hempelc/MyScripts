...
#usage: script_chris_plasmid_assembly_and_mito_extraction_and_taxonomy.sh "Reads_1" "Reads_2" "number_of_blast_hits (I set it to 5 for this purpose)"

exec 3>&1 4>&2
trap 'exec 2>&4 1>&3' 0 1 2 3
exec 1>script_chris_plasmid_assembly_and_mito_extraction_and_taxonomy_log.txt 2>&1
...
name1=$(echo ${R1##*/})
name2=$(echo ${R2##*/})
spades.py --plasmid -1 ./trimgalore_output/${name1::-6}_val_1.fq -2 ./trimgalore_output/${name2::-6}_val_2.fq -o spades_output
...
cat ./spades_output/contigs.fasta | grep '>' | cut -c 2- | sed 's/_/\t/2' | sed 's/_/\t/2' | sed 's/_/\t/2' | sed 's/_/\t/2' | sed 's/_/\t/2' | cut -f1,3,5,6 > contigs_nodenames_length_coverage_component.txt

# Merge contig length and coverage on blast output
echo -e "\n\n~~~~~~~~~~ MERGING 2 ~~~~~~~~~~\n"
mergeFilesOnColumn.pl contigs_nodenames_length_coverage_component.txt NODE_SEQID_diamond_output_mitochondrial.txt 1 1 | cut -f-4,6- > NODE_LENGTH_COVERAGE_COMPONENT_SEQID_diamond_output_mitochondrial.txt

# Get taxonomic classification of blasthits 
ete3 ncbiquery --info --search $(cut -f 18 NODE_LENGTH_COVERAGE_COMPONENT_SEQID_diamond_output_mitochondrial.txt) >matching_lineages.tsv

# Add taxonomic classification to blastoutput
echo -e "\n\n~~~~~~~~~~ ADDING TAXONOMY CLASSIFICATION ~~~~~~~~~~\n\n"
LookupTaxonDetails2.py -b NODE_LENGTH_COVERAGE_COMPONENT_SEQID_diamond_output_mitochondrial.txt -l matching_lineages.tsv -o final_output_mitochondrial_contigs_and_taxonomy.txt -t 18
...
sed -i '/^Use of/ d' script_chris_plasmid_assembly_and_mito_extraction_and_taxonomy_log.txt

# Sort files
mkdir final_output
mv NODE* matching_lineages.tsv contigs_nodenames_length_coverage_component.txt diamond_output.txt final_output_mitochondrial_contigs_and_taxonomy.txt final_output/
mkdir script_chris_plasmid_assembly_and_mito_extraction_and_taxonomy_$(echo ${name1%_*})
mv prokka_output spades_output trimgalore_output final_output script_chris_plasmid_assembly_and_mito_extraction_and_taxonomy_log.txt script_chris_plasmid_assembly_and_mito_extraction_and_taxonomy_$(echo ${name1%_*})/